...
import { OrgNavItems } from "../config";
// import { NavItems } from "../config";

export default function OrgSideNav() {
  const [isSidebarExpanded, setIsSidebarExpanded] = useState(true);
  const navItems = OrgNavItems();
  const toggleSidebar = () => {
  ...
  return (
    <div className="pr-4">
      <div
        className={cn(
          isSidebarExpanded ? "w-[210px]" : "w-[68px]",
          "border-r transition-all duration-300 ease-in-out transform hidden sm:flex h-full  bg-primary"
        )}
      >
